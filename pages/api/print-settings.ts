...
function getCampusFromCookie(req: NextApiRequest) { const cookies = parseCookie(req.headers.cookie || ''); return decodeURIComponent(cookies.st_campus || '').trim(); }
function isAuthed(req: NextApiRequest) { const cookies = parseCookie(req.headers.cookie || ''); return cookies.st_auth === '1'; }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    if (!isAuthed(req)) return res.status(401).json({ ok: false, error: 'Login required' });
    const campusKey = getCampusFromCookie(req);
  ...
  if (req.method === 'POST') {
    ...
    const campusKey = norm(req.body?.campusKey || admin.campus || getCampusFromCookie(req));
    ...
    try {
      if (privateSheetsConfigured()) {
        ...
        const kept = (existing || []).filter((row: any) => {
          const c = norm(rowVal(row, 'campusKey', 'CampusKey', 'campus', 'Campus'));
          const k = norm(rowVal(row, 'settingKey', 'SettingKey', 'key', 'Key'));
          return !(c === campusKey && incomingKeys.has(k));
        });
