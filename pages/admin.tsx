          ...
          {!error && (
            <>
              ...
              <section className="card mt-4">
                ...
                {(data?.recentPrints || []).length ? data!.recentPrints!.map((e, idx) => (
                  <div className="admin-row" key={`${e.student}-${e.ms}-${idx}`}>
                    ...
                    <span>{e.ok ? 'OK' : 'Failed'}</span>
                  </div>
                )) : <Empty>No print rows loaded. Optional: set PRINT_LOG_CSV_URL to a readable print log CSV.</Empty>}
              </section>
              ...
              <section className="card mt-4">
                ...
                )) : <Empty>No feedback rows loaded. Check FEEDBACK_PROGRESS_CSV_URL.</Empty>}
              </section>
